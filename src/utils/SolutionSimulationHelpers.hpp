...
namespace templ
{
    namespace utils
    {
        ...
        solvers::SolutionAnalysis::MinMaxModelPools getRequiredResources(const solvers::FluentTimeResource &ftr, const std::vector<solvers::FluentTimeResource> &resourceRequirements, const OrganizationModelAsk &om)
        {
        ...
        ModelPool getMinResourceRequirements(const solvers::FluentTimeResource &ftr, const std::vector<solvers::FluentTimeResource> &resourceRequirements, const OrganizationModelAsk &om)
        {
            return getRequiredResources(ftr, resourceRequirements, om).first.front();
        }

        ResourceInstance::List getAvailableResources(const symbols::constants::Location::Ptr &location,
                                                     const solvers::temporal::point_algebra::TimePoint::Ptr &timepoint, const SpaceTime::Network &solutionNetwork, const moreorg::OrganizationModelAsk &om)
        {
        ...
        ResourceInstance::List getMinAvailableResources(const SpaceTime::Network::tuple_t::Ptr &tuple, const SpaceTime::Network &solutionNetwork, const OrganizationModelAsk &om)
        {
            ResourceInstance::List availableResources = getAvailableResources(tuple->first(), tuple->second(), solutionNetwork, om);
        ...
        metrics::Probability::List initializeProbabilityList(const ProbabilityType &type, std::map<reasoning::ModelBound, ResourceInstance::List> &resourceAssignmentMap, ResourceInstance::List &available, const OrganizationModelAsk &ask, double t0, double t1)
        {
            ...
            case POF:
            {
                ...
                ProbabilityDensityFunction::Ptr defaultPDF;
                defaultPDF = make_shared<WeibullPDF>(144000., 1.);
        ...
        metrics::Probability::List matchResourcesToProbability(const ProbabilityType &type, const OrganizationModelAsk &ask, const std::vector<owlapi::model::OWLCardinalityRestriction::Ptr> &required, ResourceInstance::List &availableAgents, double t0, double t1)
        {
            if (required.empty())
            {
        ...
        solvers::temporal::TemporalConstraintNetwork::Assignment quantifyTime(templ::Mission::Ptr &mission, SpaceTime::Network &solutionNetwork, std::vector<solvers::FluentTimeResource> &resourceRequirements)
        {
        ...
        ResourceInstance::List filterResourcesByBlacklist(ResourceInstance::List &resources, ResourceInstance::List &blacklist)
        {
        ...
        ModelPool checkAndRemoveRequirements(ModelPool &available, ModelPool &required, const OrganizationModelAsk &ask)
        {
        ...
        ResourceInstance::List filterSingleResource(ResourceInstance::List &resources, ResourceInstance &resourceToRemove)
        {
            ResourceInstance::List returnList;
            for (auto &r : resources)
            {
        ...
        int checkFutureImpactOfAssignment(SpaceTime::Network &solution, templ::Mission::Ptr &mission, ResourceInstance &assignment, ResourceInstance::List &previouslyFailed, SpaceTime::Network::tuple_t::Ptr &vertexTuple, std::map<SpaceTime::Network::tuple_t::Ptr, solvers::FluentTimeResource::List> &tupleFtrMap, OrganizationModelAsk &omAsk, const std::vector<solvers::FluentTimeResource> &resourceRequirements)
        {
